...
import { DetalleEntrega } from 'src/app/models/DetalleEntrega';
...
export class EntregaCreateComponent implements OnInit {
  ...
  get_data_pedido(id:any){
    this._pedidoService.data_pedido(id).subscribe(
      response=>{
        this.detalle_pedido=response.detalle_pedido
      },
      error=>{
  ...
  save_detalle(detalleForm: { valid: any; value: { cantidad: any; idproducto: any; }; }){
    if(detalleForm.valid){
        if(detalleForm.value.cantidad >0){
          ...
          this.detalle = new DetalleEntrega('','',0);
          this.producto.stock = '--|--';
  ...
  onSubmit(entregaForm: { valid: any; value: { idpedido: string; estado:string }; }){
    if(entregaForm.valid){
      if(entregaForm.value.idpedido != ''){
        let data = {
          idpedido: entregaForm.value.idpedido,
          estado: entregaForm.value.estado,
          iduser: this.identity._id,
          detalles: this.data_detalle,
          
        }

        this._entregaService.save_data(data).subscribe(
          response =>{
